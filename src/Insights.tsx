...
function Insights(_props: InsightsProps) {
  ...
  // Fetch customer count on component mount (and potentially when dates change)
  useEffect(() => {
    const fetchCustomerCount = async () => {
      ...
      try {
        const response = await fetch('http://localhost:3001/api/individualob/count', { //Add the count route 
        ...
        setCustomerCount(data.count); // Assuming the API returns { count: number }

      } catch (error) {
        console.error('Error fetching customer count:', error);
        // Consider setting an error state here to display an error message in the UI.
      }
    };
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 mt-2">{customerCount}</p> {/* Display count here */}
        </div>
