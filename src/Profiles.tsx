...
function Profiles({ searchResults, isLoading: initialLoading }: ProfilesProps) {
  ...
  const [trackedResults, setTrackedResults] = useState<SearchResult[]>([]);  // Tracked results for display
  ...
  const filteredResults = useMemo(() => {
    const trackedSet = new Set(trackedResults.map(result => result.name));
    return searchResults.filter(result => !trackedSet.has(result.name));
  }, [searchResults, trackedResults]);

  // Memoize company matches
  const companyMatches = useMemo(() => {
    const matchSet = new Set(allPersons.map(person => person.name.toLowerCase()));
    return new Set(
      companyData
        .filter(company => 
          matchSet.has(company.company_name.toLowerCase()) ||
          Array.from(matchSet).some(name => 
            company.company_name.toLowerCase().includes(name) ||
            name.includes(company.company_name.toLowerCase())
          )
        )
        .map(company => company.company_name)
    );
  }, [companyData, allPersons]);

  // Memoize individual matches
  const individualMatches = useMemo(() => {
    const matchSet = new Set(allPersons.map(person => person.name.toLowerCase()));
    return new Set(
      individualData
        .filter(individual => 
          matchSet.has(individual.full_name.toLowerCase()) ||
          Array.from(matchSet).some(name => 
            individual.full_name.toLowerCase().includes(name) ||
            name.includes(individual.full_name.toLowerCase())
          )
        )
        .map(individual => individual.full_name)
    );
  }, [individualData, allPersons]);

  // Optimize checkForMatch with memoized sets
  const checkForMatch = useCallback((name: string) => {
    const lowerName = name.toLowerCase();
    return companyMatches.has(lowerName) || individualMatches.has(lowerName);
  }, [companyMatches, individualMatches]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const updatedCompanyData = companyDataResult.map(company => ({
        ...company,
        status: checkForMatch(company.company_name) ? 'pending' : 'approved'
      }));

      const updatedIndividualData = individualDataResult.map(individual => ({
        ...individual,
        status: checkForMatch(individual.full_name) ? 'pending' : 'approved'
      }));
  ...
  useEffect(() => {
    fetchData();
  }, [user, navigate]); // fetchData depends on user and navigate
      ...
      {!isLoading && activeTab === 'alerts' && (
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {/* First, display tracked results */}
              {trackedResults.map((result, index) => (
                <tr key={`tracked-${index}`} className="border-t border-gray-100 hover:bg-gray-50">
              ...
              {/* Then, display untracked search results */}
              {filteredResults.map((result, index) => (
                <tr key={`untracked-${index}`} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="py-4 px-6">
                    <div className={`w-1 h-6 rounded-full ${getRiskColor(result.riskLevel).replace('text', 'bg')}`}></div>
                  </td>
                  <td className="py-4 px-6">
            ...
            <table className="min-w-full">
              ...
              <tbody>
                {individualData.map((individual, index) => {
                  const matchFound = checkForMatch(individual.full_name);
                  return (
            ...
            <table className="min-w-full">
              ...
              <tbody>
                {companyData.map((company, index) => {
                  const matchFound = checkForMatch(company.company_name);
                  return (
                    <tr key={index} className="border-t border-gray-100 hover:bg-gray-50">
